...
begin
  ...
  if p_acao = 'aprovar' then
    ...
    update public.profissionais set status='rejeitado', ativo=false, destaque=false where id=p_id;
    ...
    -- Teste grátis: p_meses = nº de DIAS (padrão 7). Libera acesso completo por N dias.
    v_plano := coalesce(p_plano, (select plano from public.profissionais where id=p_id));
    update public.profissionais set ativo=true, status='aprovado', liberado_em=now(),
